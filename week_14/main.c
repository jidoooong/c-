...
void checkDie(void)
{
    ...
    for (i=0;i<N_PLAYER;i++)
    {
        if (board_getBoardStatus(player_position[i]) == BOARDSTATUS_NOK)
        {
            ...
            player_status[i] = PLAYERSTATUS_DIE; //die from shark
        }
    }
}
// ----- EX. 5 : shark ------------
...
// ----- EX. 4 : player ------------
        ...
        while (player_position[turn] >= N_BOARD) 
      {player_position[turn] -= N_BOARD;  
        }

        printf("%s rolls the dice : %d, Move position %d\n", player_name[turn], dieResult, player_position[turn]); //MOVEMENT RESULT PRINT 

        //step 2-4. coin
         int coin = board_getBoardCoin(player_position[turn]);  // take coin


         if (coin > 0) 
         {
          player_coin[turn] += coin;  // pius coin
         printf("%s add coins %d, Position %d, Total coins: %d\n", player_name[turn], coin, player_position[turn], player_coin[turn]);
        }
        ...
        //step 2-5. end process
         ...
         {
          player_status[turn] = PLAYERSTATUS_END;  // end player status
